 ...
 *
 * Matches this project's existing persistence style (SharedPreferences).
 * Keep all options OFF by default.
 */
object DevOptionsSettings {
    ...
    private const val KEY_MODERATION_OVERRIDE = "moderation_override" // OFF/CLEAN/FLAGGED/BLOCKED

    private const val KEY_FORCE_AUDIENCE = "force_audience" // null or enum
    private const val KEY_FORCE_KIDS_FILTER = "force_kids_filter" // null or enum
    ...
    fun setDevMenuEnabled(context: Context, enabled: Boolean) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putBoolean(KEY_DEV_MENU_ENABLED, enabled)
        }
    }

    fun setShowDmDebugOverlay(context: Context, enabled: Boolean) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putBoolean(KEY_SHOW_DM_DEBUG_OVERLAY, enabled)
        }
    }

    fun setDmForceSendFailure(context: Context, enabled: Boolean) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putBoolean(KEY_DM_FORCE_SEND_FAILURE, enabled)
        }
    }

    fun setDmSendDelayMs(context: Context, delayMs: Long) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putLong(KEY_DM_SEND_DELAY_MS, delayMs.coerceIn(0L, 15_000L))
        }
    }

    fun setDmDisableRateLimit(context: Context, enabled: Boolean) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putBoolean(KEY_DM_DISABLE_RATE_LIMIT, enabled)
        }
    }
    ...
    fun setModerationOverride(context: Context, override: DevModerationOverride) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putString(KEY_MODERATION_OVERRIDE, override.name)
        }
    }
    ...
    fun setForcePinSet(context: Context, enabled: Boolean) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putBoolean(KEY_FORCE_PIN_SET, enabled)
        }
    }

    fun setForcePinVerifySuccess(context: Context, enabled: Boolean) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit {
            putBoolean(KEY_FORCE_PIN_VERIFY_SUCCESS, enabled)
        }
    }
